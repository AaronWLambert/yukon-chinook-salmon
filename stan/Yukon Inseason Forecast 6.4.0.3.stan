...
//        Midpoint prediction used as prior in curve fitting for midpoint parameter (mu)
...
data {
  ...
  // April air mean temp
  // int <lower=0> n_yearAT;
  // vector [n_yearAT]apr_air_hist;
  // real  apr_air_curr;
...
parameters {
  ...
  // real <lower=160,upper=190> ps_mu_curr;
  ...
  // real <lower=160,upper=190> ps_mu_hist[n_yearPSS];
...
model {
  ...
  ps_mu_curr ~ normal((curr_predMP), sigma_env);
  ...
  // ps_alpha_curr ~ uniform(45e3,5e5);
  (ps_mu_hist) ~ normal((predMP2), sigma_env);
...
generated quantities{
  ...
  // Prediction interval for MP prediction
  post_curr_predMP = normal_rng(curr_predMP, sigma_env);
  
  post_predMP = normal_rng(predMP, sigma_env);
