// Yukon King Inseason Forecast Version 2.1.1.0
//Notes: log(EOS)~cumPSS w/ lognormal correction
//         lognormal correction added to PF
...
transformed parameters{
 ...
 // sigma_predPSS = sd(log(totalEOS[loc_eagle_years] ./ predPSS));
 
 
 // // if statement to fit intercept only before day 178
 // if(dayPSS[n_dayPSS]<178){
 //   for (i in 1:n_yearEagle){
 //  // intercept only
 //   ln_predEagle[i] = alpha_eagle;}
 //   // intercept only prediction
 //   ln_curr_predEagle = alpha_eagle;
 // }else{
