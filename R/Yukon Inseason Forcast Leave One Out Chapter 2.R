...
n.iter <- 3000;#5e4 # Number of iterations to run
...
# May mean sst for years 2000-2022
...
fit <- stan(file = file.path(dir.stan,paste("Yukon Inseason Forecast ",
            ...
            data = list("PSS_mat"=PSS_mat,
                        ...
                        "n_yearAT" = n.year.air,
                        "apr_air_hist" = april.air.hist,
                        "apr_air_curr" = april.air.curr,
...
# ord <- order(cumPSS)
# 
# # Regression plot for non-GSI-adjusted PSS passage up to myDay
...
text(x = may.sst.hist, y = hist.midpoint + 0.5, labels = yearSST, cex = .75)
points(x = may.sst.curr,
       y = logistic.all$mid[logistic.all$year==myYear] ,
       cex = 3,
       lwd = 5,
       col = "Blue",
       pch = 23,
...
text(x = may.sst.curr, 
     y = logistic.all$mid[logistic.all$year==myYear] +1,
     labels = myYear,
     cex = 2,
     col = "blue")

# Histograms of predicted mp and the actual mp

hist(pars$post_curr_predMP ,
     xlim = c(170,180))
abline(v = logistic.all$mid[logistic.all$year == myYear],
       col = "red")


# Put MP draws into a data.frame
predMP.DF <- data.frame(pars$post_predMP)

# Name the columns with the year
colnames(predMP.DF) <- yearSST

predMP.DF_long <- pivot_longer(predMP.DF,names_to = "year", values_to = "draws", cols = c(1:length(yearSST)))

predMP.DF_long$year <- as.numeric(predMP.DF_long$year)

predMP_DF_final <- left_join( predMP.DF_long, logistic.all[,c(2,5)])

ggplot(predMP_DF_final, aes(x = draws))+
  geom_vline(aes(xintercept = mid), col = "red")+
...
ggplot(masterDF, aes(x = Days,  y = count/1000))+
  ...
  # geom_col(alpha = .3)+
  geom_vline(aes(xintercept = log_mid, col = "Avg MP"), size = 1.5 , linetype = 2)+
