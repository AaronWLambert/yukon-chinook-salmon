...
//Notes: log(EOS)~meanadj_cumPSS
//      

//Components:
//  1) Pre-season forecast  (Prior)
//  2) PSS adj by GSI mean across days for current counts
...
data {
  ...
  real<lower=0> mean_adj_curr_PSS[n_curr_PSS];
  
  // Matrix of mean across days adjusted historic counts by days & years
  // int<lower=0> n_hist_counts;
  matrix<lower=0> [n_dayPSS, n_yearPSS]mean_adj_PSS_mat;
...
transformed parameters{
 ...
 for (i in 1:n_yearPSS){
   cumHistPSS[i] = sum(mean_adj_PSS_mat[,i]);
 }
 ...
 // Sum curr_PSS
 cum_current_PSS = sum(mean_adj_curr_PSS);
