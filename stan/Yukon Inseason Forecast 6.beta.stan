...
data {
  ...
  matrix <lower=0,upper=1> [n_dayPSS_all, n_yearPSS]PSS_mat_prop_all;
  ...
  // April air mean temp
  int n_sst;
  vector [n_sst]may_sst_hist;
  real  may_sst_curr;
  
  // Historical deviations from avg midpoint timing
  vector[n_sst]dev_hist;
  // real avg_midpoint;
...
transformed parameters{
  ...
  vector [n_dayPSS_all]A;
  
  vector [n_dayPSS_all]B;
  ...
  for(d in 1:n_dayPSS_all){
    ...
    A[d] = ps_prop_pred[d] * phi;

    B[d] = (1-ps_prop_pred[d]) * phi;}
...
model {
  ...
  // for(d in 1:n_dayPSS_all){
  //   for(y in 1:n_yearPSS){
  //     if(PSS_mat_prop_all[d,y]>0 && PSS_mat_prop_all[d,y]<1){
  // 
  //   PSS_mat_prop_all[d,y]  ~ beta(A[d], B[d]);}}
  //   // target += beta_lpdf(PSS_mat_prop_all[d,y]|A[d],B[d]);
  //   }
   for(d in 1:n_dayPSS_all){
    for(y in 1:n_yearPSS){
      if(PSS_mat_prop_all[d,y]>0 && PSS_mat_prop_all[d,y]<1){
