...
# Purpose: Function that will run the model and return posterior parameter estimates
# 
#
#   1) Read in data
...
#=================================================================================
#NOTES:
# This version is the initial version that future models will be built from
#
# 
# Next steps: 
# 
...
InSeasonProjection <- function(model.version,
  ...
  outs <- list("cumPSS" = cumPSS, "pars" = pars, "totalEOS" = totalEOS, "fit" = fit)
  }
...
# parsList<-InSeasonProjection(CAN_hist = CAN_hist,
#                              pf_hist = pf_hist,
#                              PSS_hist = PSS_hist,
#                              myDay = myDay,
#                              myYear = myYear,
...
# Loop for multiple years and days
# Plots for paper
outputList<-list()
for(y in 2019){
  for(d in c(153,179,196)){
    
    outputList[[paste("",y,"_",d, sep = "")]]<-InSeasonProjection(model.version = "2.0",
                                                       myYear = y,myDay = d,
                                                       n.chains = 4,
                                                       CAN_hist = CAN_hist,
                                                       pf_hist = pf_hist,
...
for (i in 1:length(outputList)) {
    ...
    pars <- outputList[[i]]$pars
    fit <- outputList[[i]]$fit
    cumPSS <- outputList[[i]]$cumPSS
    totalEOS <- outputList[[i]]$totalEOS
    ...
    outputPlots[[names(outputList[i])]] <- list("DensPlot" = postDense, "PredPlot"= zz)
...
outputList$
pars <- parsList$pars
cumPSS <- parsList$cumPSS
totalEOS <- parsList$totalEOS
...
ggplot(df.comb, aes(x = value/1000, fill = par))+
  stat_eye(position = "dodge", color = "grey")+
...
for(y in 2019){
  for(d in c(153,179,196)){
    ...
    output.test.version2[[paste(y,"_",d, sep = "")]]<-InSeasonProjection(model.version = "2.0",
                                                                                myYear = y,
                                                                                myDay = d, 
                                                                                n.chains = 4, 
                                                                                CAN_hist = CAN_hist, 
...
ggarrange(output.test[[1]][[2]],
          ...
          ncol = 1, nrow = 3, 
          common.legend = TRUE,
          # labels = c("June 4 2017 ","July 4 2017","Aug 3 2017",
          "June 4 2018","July 4 2018","Aug 3 2018",
