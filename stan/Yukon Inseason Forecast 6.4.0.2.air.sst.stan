...
data {
  ...
  // April air mean temp
  int <lower=0> n_yearAir;
  vector [n_yearAir]april_air_hist;
  real  april_air_curr;
  
  // Historical midpoint timing
  vector <lower=0> [n_yearSST]hist_MP;
...
parameters {
  ...
  // Regression parameters for midpoint regression using SST and air temp
  real alpha_env;
  real beta_sst;
  real beta_air;
...
transformed parameters{
  ...
  vector [n_yearSST]predMP;
  
  vector [n_yearPSS]predMP2;
  
  real curr_predMP;
  
  
  // Predicted MP
  predMP = alpha_env + beta_sst * may_sst_hist + beta_air * april_air_hist;
  
  curr_predMP = alpha_env + beta_sst * may_sst_curr + beta_air * april_air_curr;
  ...
  predMP2 = predMP[loc_devMP_allYears];
  
  // Curve fit to days up to myDay for the current year of interest
  ps_pred_curr_obs[1:n_dayPSS] = ps_alpha_curr * (1/(ps_sd_curr*sqrt(2*pi())))*exp(-0.5*square((dayPSS - curr_predMP)/ps_sd_curr));
  
  // Curves fit to complete and partial historic passage by year
  for(y in 1:n_yearPSS){
    for(d in 1:n_dayPSS_all)
    
    ps_pred_hist[d,y] = ps_alpha_hist[y] * (1/(ps_sd_hist[y]*sqrt(2*pi())))*exp(-0.5*square((dayPSS_all[d] - predMP2[y])/ps_sd_hist[y]));

    for(d in 1:n_dayPSS){
      
        ps_pred_hist_obs[d,y] = ps_alpha_hist[y] * (1/(ps_sd_hist[y]*sqrt(2*pi())))*exp(-0.5*square((dayPSS[d] - predMP2[y])/ps_sd_hist[y]));
    }
   ...
   // Used to calculate empirical sd in update
    ...
    //    cum_current_Eagle = sum(curr_Eagle);
       ...
       curr_predEagle = (cum_current_Eagle + 1e-3)/mean_propEagle;
  
       predEagle =  (cumHistEagle + 1e-3)/mean_propEagle;
...
model {
  ...
  // ps_mu_curr ~ normal(curr_predMP, sigma_sst);
  ...
  // ps_alpha_curr ~ uniform(45e3,5e5);
  // ps_mu_hist ~ normal(predMP[loc_devMP_allYears], sigma_sst);
  ...
  alpha_env ~ normal(0,1e15);
  beta_sst ~ normal(0,1e15);
  beta_air ~ normal(0,1e15);
  sigma_env ~ normal(0,1e6);
  
  // Preseason Forcast likelihood
  ln_RunSize ~ normal(Pf, Pf_sigma);
  
  hist_MP ~ normal(predMP, sigma_env);
...
generated quantities{
  ...
  // post_curr_predMP = normal_rng(curr)
