// Yukon King Inseason Forecast Version 5.1
...
model {
  ...
  ps_mid_curr ~ normal(mean(ps_m), sd(ps_m)/3);
  ...
  ps_mid_hist ~ normal(mean(ps_m), sd(ps_m)/3);
  ...
  log(totalEOS[1:n_yearPSS])~normal(ln_predPSS[1:n_yearPSS]-sigma_reg^2/2, sigma_reg);
