...
transformed parameters{
  ...
  for(d in 1:n_dayPSS_all){
   ...
   // ps_pred_curr = ps_alpha_curr *(1/(1 + exp((-(dayPSS_all - ps_mid_curr)) / ps_shape_curr)));
  ...
  for(y in 1:n_yearPSS){
    ...
    for(y in 1:n_yearPSS){
    cum_predicted_histPSS[y] = cumHistPSS[y] + (ps_pred_hist[n_dayPSS_all,y] - ps_pred_hist[(n_dayPSS),y]);}
    ...
    sigma_predPSS = sd(log(totalEOS[loc_pf_years_PSS] ./ predPSS_hist[loc_pf_years_PSS]));
      ...
      // sigma_predPSS = sd(log(totalEOS) - (ln_predPSS_hist));
      
    // Sum curr_PSS
    cum_current_PSS = sum(curr_PSS) + (ps_pred_curr[n_dayPSS_all] - ps_pred_curr[n_dayPSS]);  
